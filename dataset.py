...
class SignFiDataset(Dataset):
	def __init__(self, root_dir, type, env, link='all', mode='single', portion=12, return_remaining=False):
		...
		if env == 'lab_same' and type == 'train':
			...
			if portion is not None and type == 'train' and link == 'all' and mode == 'single':
				...
				# if type == 'train' and portion != 12:
				# 	classes = np.unique(self.label)
				# 	new_csi = None
				# 	new_label = None
				# 	for i in range(len(classes)):
				# 		indices = np.where(self.label == classes[i])[0]
				# 		np.random.shuffle(indices)
				# 		indices = indices[:portion]
				# 		if new_csi is None:
				# 			new_csi = self.csi[:,:,:,indices]
				# 			new_label = self.label[indices]
				# 		else:
				# 			new_csi = np.concatenate((new_csi, self.csi[:,:,:,indices]), axis=3)
				# 			new_label = np.concatenate((new_label, self.label[indices]), axis=0)
				# 	self.csi = new_csi
				...
				# classes = np.unique(self.label)
				# num_samples = np.zeros(len(classes))
				# for i in range(len(classes)):
				# 	num_samples[i] = np.sum(self.label == classes[i])
				# print(f'Number of samples per class: {num_samples}')
				# print(f'Averaged number of samples per class: {np.mean(num_samples)}')
	...
	def __getitem__(self, idx):
		# if self.mode == 'single':
		# 	return torch.FloatTensor(self.csi[:,:,:,idx]), self.label[idx].astype('int64')
		# elif self.mode == 'dual':
		# 	return torch.FloatTensor(self.csid[:,:,:,idx]), torch.FloatTensor(self.csiu[:,:,:,idx]), self.label[idx].astype('int64')
		# else:
		# 	raise ValueError('Invalid mode type')
		if self.mode == 'single':
