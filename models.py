...
class SSLModel(pl.LightningModule):
	...
	def dual_augmentaion(self, batch):
		if 'SignFiDual' in self.hparams.model['augmentations'] and self.hparams.model['augmentations']['SignFiDual']:
			real_x1, real_x2, y = batch
			x1 = real_x1
			x2 = real_x2
		elif 'SignFiRandomDual' in self.hparams.model['augmentations'] and self.hparams.model['augmentations']['SignFiRandomDual']:
			tmp1, tmp2, y = batch
	...
	def augmentation(self, x):
		...
		if self.hparams.model['type'] != 'RecurrentRSCNet' and 'zero_masking' in self.hparams.model['augmentations'] and self.hparams.model['augmentations']['zero_masking']:
	...
	def training_step(self, batch, batch_idx):
		if self.trainer.global_step == 0 and not self.hparams.debug: 
			wandb.define_metric('train_loss', summary='min')

		x1, x2, real_x1, real_x2, y = self.dual_augmentaion(batch)

		x1, x2 = self.augmentation(x1), self.augmentation(x2)

		batch_size = y.shape[0]
		tmp_batch_size = batch_size
		if self.hparams.model['type'] == 'RecurrentRSCNet':
...
class LinearClassifierModel(pl.LightningModule):
	def __init__(self, pretrained_encoder, hparams):
		...
		# Check if partial freezing of the encoder is required
		if self.hparams.partial_freeze_encoder:
			# Freeze the parameters of the encoder's encoder
			for param in self.encoder.encoder.parameters():
				param.requires_grad = False
		# Check if the model is not semi-supervised
		elif not ('semi_supervised' in self.hparams and self.hparams['semi_supervised']):
	...
	def forward(self, x):
		if isinstance(self.encoder, RecurrentEncoder) or (self.hparams.supervised and self.hparams.model['type'] == 'RecurrentRSCNet'):
